...
<div class="top-part" id="sampleFrame">
    <div class="clearfix">
         ...
         <?php
                    if(isset($session['user_id']) && $session['user_id']!='' && isset($session['user_name']) && $session['user_name']!='')
                    {
                        ...
                        <input type="hidden" name="profile_pic" value="<?php echo $profile_pic;?>" id="">
                         <input type="hidden" name="msgcount" id="msgcount" value="">
                         <!-- <iframe id="sampleFrame" src="https://www.bitdegree.org/"></iframe> -->
        ...
        <div class="col-sm-3 col-xs-6 padding-lft-10">
            <div class="top-part-box">
                ...
                <div class="up-circle spl-shade-1">
                    <img src="<?= Yii::$app->request->baseUrl; ?>/frontend/images/icon_message.png" class="media-object icon-img">
                    <h1 class="spl-shade-2"> <p id="exam"></p></h1>
                </div>
                <h1>
                    <a href="javascript:;">Messaging</a> 
                </h1>
            </a>
            </div>
        </div>
        ...
        <div class="col-sm-3 col-xs-6 padding-lft-10">
            <div class="top-part-box">
                <a href="<?= Url::toRoute(['family/editprofile']); ?>">
                <div class="up-circle spl-shade-2">
                    <img src="<?= Yii::$app->request->baseUrl; ?>/frontend/images/icon_profile.png" class="media-object icon-img">
                    <!--                                                                <h1>4.5</h1>-->
                </div>
                </a>
                <h1>
                    <a href="<?= Url::toRoute(['family/editprofile']); ?>">Profile</a> 
                </h1>
            </div>
        </div>
        ...
        <div class="col-sm-3 col-xs-6 padding-lft-10">
            <div class="top-part-box">
                <a href="<?= Url::toRoute(['family/dashboard']); ?>">
                <div class="up-circle spl-shade-2">
                    <img src="<?= Yii::$app->request->baseUrl; ?>/frontend/images/icon_log_a_job.png" class="media-object icon-img">
                    <!--                                                                <h1>4.5</h1>-->
                </div>
                </a>
                <h1>
                    <a href="<?= Url::toRoute(['family/dashboard']); ?>">Jobs</a> 
                </h1>
            </div>
        </div>
        ...
        <div class="col-sm-3 col-xs-6 padding-lft-10">
            <div class="top-part-box">
                <div class="up-circle spl-shade-3">
                    <img src="<?= Yii::$app->request->baseUrl; ?>/frontend/images/icon_wallet.png" class="media-object icon-img">
                    <!--                                                                <h1>4.5</h1>-->
                </div>
                <h1>
...
<script type="text/javascript">
    ...
    if(userId!='' && userId!=0 && nickname!='' && nickname!=0)
    {
  ...
  if (unread != 0) {
   // $.each($('.left-nav-channel'), function (index, item) {       
      ...
      // if ($(item).data("channel-url") == targetUrl) {
        //alert('hii');
        addUnreadCount(unread, targetUrl);
      //}
    //});
}
});
...
// function startSendBird1(userId, cname) {
//   sb = new SendBird({
//     appId: appId
//   });
...
//     var nickName = decodeURI(decodeURIComponent(getUrlVars()['cname']));
...
function startSendBird(userId, nickName) {
    ...
    var initPage = function (user) {
        ...
        currentUser = user;
        //alert(pic);
        sb.updateCurrentUserInfo(nickName, pic, function (response, error) {
  // console.log(response, error);
});
...
function addUnreadCount(unread, targetUrl) {

    var count = unread;
    //document.getElementById("sampleFrame").onload = function() {sampleFunction()};

//function sampleFunction() {
    //alert("hii");
 document.getElementById("exam").innerHTML = count;
    //alert(count);
    var message_count = count;
    <?php $abc = "<script>document.write(message_count)</script>"?>
    document.getElementById("msgcount").value = count;
    //alert(document.getElementById("msgcount").value);
...
// }
    ...
    if(unread>=1)
    {
       /* if (Notification.permission !== "granted")
            ...
            var notification = new Notification('New Message', {
                icon: '/web/frontend/images/logo.png',
                body: "Hey there! You've been notified!",
            });
...
function init() {
    ...
    nickname = document.getElementsByName('user_name')[0].value;*/
...
<script type="text/javascript">
    function chat()
    {
       ...
       // alert(userId);
        ...
        window.location.href = '/joblog/chatview1?userid=' + encodeURIComponent(userId) + '&nickname=' + encodeURIComponent(nickname) + '&pic=' + encodeURIComponent(pic);
