...
function changeImage() {
    $('#w0').remove();
    var url = full_path + 'myprofile/adimage';
    var href = $(".editor").find('img').attr('src');
    $.ajax({
        type: 'POST',
        url: url,
        data: {g: href},
        ...
        success: function (data) {
            if (data.res == 1) {
                ...
                $("#dispalyImageP").attr('src', data.imgUrlP);
                $("#dispalyImageOriginal").attr('data-photo', data.imgUrlO);
                $("#dispalyImageDiv").show();
                $("#deleteImageOuter").css('display', 'inline-block');
                $("#deleteImage").show();
                $("#cropper").modal('hide');
            } else {
                $("#cropper").modal('hide');
                var err = '<div id="w0" class="alert-danger alert fade in">' +
                        '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' +
                        'Please select an image.' +
                        '</div>';
                $('.breadcrumb').after(err);
            }
        }
    });
}
...
function deleteImage() {
    $("#dispalyImageP").attr('src', '');
    $("#dispalyImageOriginal").attr('src', base_path + 'backend/assets/pages/img/admin-default.jpg');
    $("#deleteImage").hide();
}

function deleteImageOuter() {
    $("#dispalyImageP").attr('src', base_path + 'backend/assets/pages/img/admin-default.jpg');
    $("#dispalyImageOriginal").attr('data-photo', base_path + 'backend/assets/pages/img/admin-default.jpg');
    $("#deleteImageOuter").hide();
}

function showPortfolio(obj) {
    var img = $(obj).attr('data-photo');
    $("#dispalyImageOM").attr('src', img);
...
function uploadBlogImage() {
    $('#error').remove();
    var url = full_path + 'blogs/blogimage';
    var href = $(".editor").find('img').attr('src');
    $.ajax({
        type: 'POST',
        url: url,
        ...
        success: function (data) {
            if (data.res == 1) {
                ...
                $("#deleteImageOuter").css('display', 'inline-block');
                $("#deleteImage").show();
                $("input#image").val(data.imgUrlO);
                $("#cropper").modal('hide');
            } else {
                $("#cropper").modal('hide');
                var err = '<div id="error" class="alert-danger alert fade in">' +
                            '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' +
                            'Please select an image.' +
                          '</div>';
                $('.breadcrumb').after(err);
        ...
        success: function (resp) {
            ...
            if (resp.flag == true) {
                if (resp.imgError == true) {
                    ...
                    var succ = '<div class="alert-success alert fade in">' +
                        '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' +
                        resp.msg +
                        '</div>';
                $('.breadcrumb').after(succ);
                 if(resp.childcare){
                 $('.adv_childcare').show();
             }else{
                 $('.adv_childcare').hide();
                 ...
                 setTimeout(function(){
                     $('html, body').animate({
                        scrollTop: $(".portlet-title").offset().top-200
                    }, 1000);
                 },100)
                }
            } else {
                ajaxindicatorstop();
                var succ = '<div class="alert-danger alert fade in">' +
                        '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' +
                        resp.msg +
                        '</div>';
                $('.breadcrumb').after(succ);
...
$('form#family-info-form111').submit(function (e) {
    $('.alert').remove();
    e.preventDefault();
    var _this = $(this);
    ajaxindicatorstart();
...
$('form#care-info-form111').submit(function (e) {
    $('.alert').remove();
    e.preventDefault();
    var _this = $(this);
    ajaxindicatorstart();
