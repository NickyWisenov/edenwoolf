...
var pic = decodeURI(decodeURIComponent(getUrlVars()['pic']));
...
$('.chat-top__button-block').click(function () {
  ...
  var blockuserid = document.getElementById('block').value;
  var bub = document.getElementById('block_status').value;
  //alert(bub);
    ...
    if (bub == 0) 
    {
      modalConfirm('Are you Sure?', 'Do you want to block this user?', function () {
       ...
       sb.blockUserWithUserId(blockuserid, function (response, error) {
          if (error) {
            alert(error);
          } else {
            document.getElementById('block_status').value = 1;
            var bub = 1;
            /*alert("hello");*/
            //alert(isBlock);
          }
  ...
  else
  {
    modalConfirm('Are you Sure?', 'Do you want to unblock this user?', function () {
       sb.unblockUserWithUserId(blockuserid, function (response, error) {
          if (error) {
            
            //alert("hoo");
          } else {
             /*document.getElementById('block').value = 0;   */
             document.getElementById('block_status').value = 0;
             var bub = 0;
            //alert("hellooo");
          }
        });
...
$('.profile').show();
  ...
  sb.GroupChannel.getChannel(channelUrl, function (channel, error) {
    ...
    $.each(members, function (index, member) {
      if (!isCurrentUser(member.userId)) {
        var fields = member.nickname.split(',');
          document.getElementById('block').value = member.userId;
          document.getElementById("myImg").src = member.profileUrl;
          document.getElementById("exam").innerHTML= fields[0];
          document.getElementById("location").innerHTML= fields[1]+' ,'+fields[2];
       
        /*document.getElementById("suburb").innerHTML= fields[2];*/
       
        channelTitle += xssEscape(fields[0]) + ', ';
      }
    });
...
function endMessaging(channel, obj) {
  ...
  $('.modal-leave-channel-desc').html('Do you want to Delete chat?');
  $('.modal-leave-channel').show();
  return false;
}
