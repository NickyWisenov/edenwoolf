...
(function ($) {
    ...
    $(document).on('click', '[data-action="like"]', function (event) {
        ...
        $.ajax({
            ...
            type: 'UPDATE',
            error: function (xhr, status, error) {
                notie.alert({type: 2, text: 'Something went wrong', time: 3});
            },
            success: function (result, status, xhr) {
                $this.parents('[data-comment-content-id="' + $this.data('comment-id') + '"]').find(".likes-num").text(result + " likes");
                notie.alert({type: 1, text: 'You liked the comment', time: 3});
            }
        });
    });

    /**
     * Report comment event
     */

    $(document).on('click', '[data-action="report"]', function (event) {
        event.preventDefault();
        var data = $.data(document, 'comment');
        ...
        $.ajax({
            ...
            success: function (result, status, xhr) {
                notie.alert({type: 2, text: 'You reorted the comment', time: 3});
            }
        });
    });
