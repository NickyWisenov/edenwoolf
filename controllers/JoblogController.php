...
//        $carerId = (Yii::$app->session["carer_id"] != "" && Yii::$app->session["carer_id"] > 0) ? trim(session["carer_id"]) : 0;
     ...
     public function actionChatview() {
        ...
        return $this->renderPartial('chat.html');
    }
     public function actionChatview1() {
        
        return $this->renderPartial('whole_chat/chat.php');
    }
     public function actionChatcheck() {
        
        if (Yii::$app->user->isGuest) {
            Yii::$app->session->setFlash('error', Yii::t("app", 'Please login to chat'));
            return $this->redirect(Yii::$app->request->referrer ? Yii::$app->request->referrer : Yii::$app->homeUrl);
        }
    }
